...
noveBits = True #Define se o imediato será de 8 ou 9 bits
...
def get_label_first(line):
    line = line.split('%')
    return line[1]

def get_label_after(line):
    line = line.split(':')
    return line[0]
...
with open(outputBIN, "w+") as f:  #Abre o destino BIN
    ...
    for line in lines:    
        if ("%" in line):
            ...
            agora = agora.replace("%","")
            agora = agora.replace("\n","")
            label_list.append(agora)
        elif (":" in line and "%" not in line):
            onde = get_label_after(line)
            onde = onde.replace(":","")
            onde = onde.replace("\n","")
            label_dict[onde] = label_cont+1
        if (line == "\n"):
            label_cont -= 1
        label_cont += 1
